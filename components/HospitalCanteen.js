...
const HospitalCanteen = () => {
  const navigation = useNavigation();
  const canteenData = [
    { id: '1', name: 'NESCAFE', status: 'Open', deliveryTime: '12 mins' },
    { id: '2', name: 'NORTH INDIA DISTRIBUTORS', status: 'Closed', deliveryTime: '18 mins' },
  ];

  const handleCanteenPress = (canteen) => {
    navigation.navigate('Menu', { canteenName: canteen.name }); // Navigate to Menu with canteen name
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Canteens for APOLLO</Text>
      <FlatList
        data={canteenData}
...
export default HospitalCanteen;
